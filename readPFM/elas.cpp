...
static void drawCircle( cv::Mat img, cv::Point center )
{
 ...
 cv::circle( img, center, 2, Scalar( 140, 255, 12 ), thickness, lineType );
}
//draw a line
static void drawLine( cv::Mat img, cv::Point start, cv::Point end )
{
  int thickness = 0.5;
  int lineType = LINE_8;
  cv::line( img, start, end, Scalar( 255, 122, 123 ), thickness, lineType );
}

//draw  triangle
static void drawTriangle( cv::Mat img, cv::Point p1, cv::Point p2,cv::Point p3 )
{
  drawLine(img,p1,p2);
  drawLine(img,p2,p3);
  drawLine(img,p3,p1);
...
//I1 & I2 are left and right images. 
//D1 & D2 are empty matrices for Disparity Images
void Elas::process (uint8_t* I1_,uint8_t* I2_,float* D1,float* D2,const int32_t* dims,cv::Mat leftImg){
  ...
  vector<support_pt> p_support = computeSupportMatches(desc1.I_desc,desc2.I_desc);
   ...
   //###** printing all the support points.. Here... ##Abhishek
  //step-1: Print the support points
   cout<<"Support points****: "<<p_support.size()<<endl;
   cv::namedWindow("Left Image", cv::WINDOW_AUTOSIZE);
  for (int i=0; i<p_support.size(); i++)
  {   Point p= Point(p_support.at(i).u ,p_support.at(i).v);
     drawCircle(leftImg,p);
       cout<< p_support.at(i).u <<" "<<p_support.at(i).v<<" "<<p_support.at(i).d<<endl; 
  } 
  
  //step 2.: Mark them on the image
 //  cv::imshow("Left Image",leftImg);
 //  cv::waitKey(0);

 //############# *** Done printing support points
...
#endif
   cout<<"Tringle Points:** left:"<<endl;
  vector<triangle> tri_1 = computeDelaunayTriangulation(p_support,0,leftImg);
  cout<<"Tringle Points:** right:"<<endl;
  vector<triangle> tri_2 = computeDelaunayTriangulation(p_support,1,leftImg);
  
  ///############ print triangles
  cout<<"Trianlge..** "<<tri_1.size()<<endl;
 /* for (int i=0; i< tri_1.size(); i++)
  {    
        //cout<< tri_1.at(i).c1 <<" "<<tri_1.at(i).c2<<" "<<tri_1.at(i).c3<<endl;
        //cout<< tri_1.at(i).t1a <<" "<<tri_1.at(i).t1b<<" "<<tri_1.at(i).t1c<<endl;
        //cout<< tri_1.at(i).t2a <<" "<<tri_1.at(i).t2b<<" "<<tri_1.at(i).t2c<<endl;
...
vector<Elas::support_pt> Elas::computeSupportMatches (uint8_t* I1_desc,uint8_t* I2_desc) {
  ...
  for (int32_t u_can=1; u_can<D_can_width; u_can++)
    for (int32_t v_can=1; v_can<D_can_height; v_can++)
      ...
      //  cout<<u_can<<" "<<v_can<<endl;///#########################################**** Abhishek
      //  show the support points here... 
...
vector<Elas::triangle> Elas::computeDelaunayTriangulation (vector<support_pt> p_support,int32_t right_image,cv::Mat img) {
  ...
  if (!right_image) {
    ...
    for (int32_t i=0; i<p_support.size(); i++) {
      in.pointlist[k++] = p_support[i].u-p_support[i].d;    ////########### important ##abhishek
      in.pointlist[k++] = p_support[i].v;
    }
  }
 ...
 ////############# abhishek draw the triangels on left image
  cv::Point p1=cv::Point(out.trianglelist[0],out.trianglelist[1]);
  if(!right_image){
  k=0; 
  cout<<"Tri**"<<endl;
          for (int32_t i=0; i<out.numberoftriangles; i++) {
             cv::Point p1=cv::Point(p_support[out.trianglelist[k]].u,p_support[out.trianglelist[k]].v);
             cv::Point p2=cv::Point(p_support[out.trianglelist[k+1]].u,p_support[out.trianglelist[k+1]].v);
             cv::Point p3=cv::Point(p_support[out.trianglelist[k+2]].u,p_support[out.trianglelist[k+2]].v);
             drawTriangle(img,p1,p2,p3);
               
              k+=3;
          }
            cv::imshow("Left Image",img);
            cv::imwrite("left_disp.png",img);
            cv::waitKey(0);
  }
